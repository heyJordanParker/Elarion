	...
	public class CurveEditorTools : EditorWindow {
		...
		bool CompareCurves(AnimationCurve C1, AnimationCurve C2) {
			...
			for(int i = 0; i < C1.keys.Length; i++) {
				if(C1.keys[i].inTangent != C2.keys[i].inTangent)
					return false;
				if(C1.keys[i].outTangent != C2.keys[i].outTangent)
					return false;
				if(C1.keys[i].time != C2.keys[i].time)
					return false;
				if(C1.keys[i].value != C2.keys[i].value)
					return false;
			}
			return true;
		}
