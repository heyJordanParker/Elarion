...
namespace Elarion.Tools.FiniteStateMachine {
    [Serializable]
    public class FiniteStateMachine : MonoBehaviour {
        ...
        public void GoTo(State state) {
            ...
            while(exitIterator != null && exitIterator != transform.parent &&
                  !state.transform.IsChildOf(exitIterator)) {
        ...
        private Stack<State> StateStack => _stateStack ?? (_stateStack = new Stack<State>());

        public State CurrentState => StateStack.Count == 0 ? _currentState : StateStack.Peek();
    }
}